...
if True:
    ...
    set_env_var("WALYTIS_BETA_API_TYPE", "WALYTIS_BETA_BRENTHY_API", override=False)

    set_env_var(
        "WALYTIS_BETA_LOG_PATH", os.path.join(os.getcwd(), "Walytis.log"), override=True
    )
    ...
    if not are_we_in_docker():
        assert_is_loaded_from_source(SRC_DIR, endra)

    endra.log.console_handler.setLevel(logging.DEBUG)
