...
RUN for SUBFOLDER in /opt/Endra/tests/endra_docker/python_packages/*; do [ -e "$python_package" ] || continue; pip install --break-system-packages --root-user-action ignore -e "$SUBFOLDER"; done
